       ...
       // returnTicketsList = bundle.getParcelableArrayList<OriginDestinationOptionItemViewModel>("as")
        ...
        filterButton.setOnClickListener(View.OnClickListener {
            if(filterSpinner.selectedItem.toString().equals("Price(Lowest)")){
                if(returnTicketsAdapter.ticketsList[0].roundtripAirItineraryPricingInfo.itinTotalFare.baseFare.amount.toFloat()>
                        returnTicketsAdapter.ticketsList[returnTicketsAdapter.ticketsList.size-1].roundtripAirItineraryPricingInfo.itinTotalFare.baseFare.amount.toFloat()) {
                    returnTicketsAdapter.ticketsList.reverse()
